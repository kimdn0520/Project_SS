...
- Hold the circular DIG face to accelerate mining. The central shaft descends vertically: the miner briefly falls and settles while seven authored vein rows recycle upward. DIG and navigation remain stationary. Input pauses during the short descent.
...
Mining console revision: seven preplaced rocks form a ring buffer that extends below the viewport. NavigationDeck is removed. A cached pickaxe icon moves with the button face, while the bottom-mounted housing and the gold-rimmed vertical burst gauge remain fixed. Chest animation switches from the exterior lid to its inside face at the hinge turn, claims a single reward at 0.78s, then descends after 1.9s. Reward items use the existing PoolManager and return after reveal.
