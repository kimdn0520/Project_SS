...
namespace ProjectSS.Expedition.Editor
{
    public static class SidebarShortcutArt
    {
        ...
        public static void Configure(PlayPage page)
        {
            ...
            for(int i=0;i<2;i++)
            {
                string name=i==0?"SidebarStore":"SidebarFirstCharge";var parent=page.minePanel.transform;
                var old=parent.Find("SidebarSlot_0_"+i);if(old!=null)Object.DestroyImmediate(old.gameObject);
                ...
                var icon=Image(vr,"Icon",7,-3,70,70,AssetDatabase.LoadAssetAtPath<Sprite>(Icons+(i==0?"UI_Shop_05_Purple.png":"UI_Rewards_Gift_02_Yellow.png")),Color.white);icon.preserveAspect=true;
                Image(vr,"NamePlate",5,57,74,23,panel,Hex("17212A")).type=UnityEngine.UI.Image.Type.Sliced;
                var label=new GameObject("Label",typeof(RectTransform),typeof(TextMeshProUGUI));label.transform.SetParent(vr,false);var text=label.GetComponent<TextMeshProUGUI>();text.font=page.depthLabel.font;text.fontSize=20;text.fontStyle=FontStyles.Bold;text.text=i==0?"스토어":"첫 충전";text.alignment=TextAlignmentOptions.Center;text.color=Hex("FFF5DC");text.raycastTarget=false;text.textWrappingMode=TextWrappingModes.NoWrap;
                ...
                var instance=(GameObject)PrefabUtility.InstantiatePrefab(prefab,parent);((RectTransform)instance.transform).anchoredPosition=new Vector2(16,-(417+i*99));
            }
        }
    }
}
