...
public static class ApplyCompactConfirmation {
 ...
 const string common="Assets/Resources/Prefabs/Popups/PopupCommon.prefab";var root=PrefabUtility.LoadPrefabContents(common);
 ...
 foreach(var name in new[]{"ExpeditionNotice","ItemDetails","EquipmentSelection","VeinSelection"}){
 string path="Assets/Resources/Prefabs/Popups/"+name+".prefab";root=PrefabUtility.LoadPrefabContents(path);
 try{var chrome=root.GetComponentInChildren<CommonPopupChrome>(true);if(chrome==null)throw new Exception("Missing inherited PopupCommon chrome: "+name);
 ...
 const string target="Assets/Resources/Prefabs/Popups/DismantleConfirmation.prefab";
 root=(GameObject)PrefabUtility.InstantiatePrefab(AssetDatabase.LoadAssetAtPath<GameObject>("Assets/Resources/Prefabs/Popups/ExpeditionNotice.prefab"));
 try{root.name="DismantleConfirmation";var notice=root.GetComponent<ExpeditionNotice>();var so=new SerializedObject(notice);so.FindProperty("popupName").stringValue="DismantleConfirmation";
 ...
 var window=root.transform.Find("PopupCommon/Window");Rect((RectTransform)window.Find("bg"),110,500,500,280);window.Find("Title/TitlePlate").gameObject.SetActive(false);
 ...
 AssetDatabase.SaveAssets();return "Compact 500x280 confirmation variant saved; close alignment inherited from PopupCommon.";
 }}
