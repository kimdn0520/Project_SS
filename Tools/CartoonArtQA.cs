...
public static class CartoonArtQA
{
 ...
 static async UniTaskVoid Run()
 {
  ...
  try
  {
   ...
   File.WriteAllText("PrototypeQA/cartoon-art.txt","PASS: two prefab shortcuts raycast + press/release, no click actions or popups. Five fracture frames. 24 scrolling descents, bottom coverage and fixed map object count. Cartoon chest hinge/inside/reward, exactly one reward.");Debug.Log("CARTOON ART QA PASS");
  }
  catch(Exception e){File.WriteAllText("PrototypeQA/cartoon-art.txt","FAIL: "+e);Debug.LogException(e);}
 ...
 static void Shot(string name)
 {
  ...
  try{cam.targetTexture=rt;cam.rect=new Rect(0,0,1,1);Canvas.ForceUpdateCanvases();cam.Render();RenderTexture.active=rt;tex.ReadPixels(new Rect(0,0,720,1280),0,0);tex.Apply();File.WriteAllBytes("PrototypeQA/cartoon-"+name+".png",tex.EncodeToPNG());}
