...
public static class VerticalPlayQA
{
    ...
    {
        var page=UnityEngine.Object.FindFirstObjectByType<PlayPage>();string backup=JsonUtility.ToJson(page.Model.Data);EquipmentSelectionPopup equipment=null;
        try
        {
            ...
            page.OpenMenu(1);page.OpenHero(0);page.SelectSlot(0);await UniTask.Delay(400);Shot("equipment-weapons");equipment=(EquipmentSelectionPopup)PopupManager.CurrentPopup;
            Check(equipment.IsOpen&&page.ActiveTab==1&&page.pausePolicy.IsPaused,"Slot did not open modal over hero");
            for(int i=0;i<equipment.rows.Length;i++)if(equipment.rows[i].root.gameObject.activeSelf)Check(page.catalog.gear[i].equipSlot==0&&(page.catalog.gear[i].hero<0||page.catalog.gear[i].hero==0),"Equipment list class/slot filter failed");
            equipment.OnClickClose();await UniTask.Delay(350);
            ...
            page.SelectSlot(2);await UniTask.Delay(350);equipment.rows[candidate].button.onClick.Invoke();await UniTask.Delay(350);
            Check(page.Model.Equipped(0,2)==candidate&&!equipment.IsOpen&&page.ActiveTab==1,"Select equipment failed");Shot("hero-equipped");
            page.SelectSlot(2);await UniTask.Delay(350);equipment.unequip.onClick.Invoke();await UniTask.Delay(350);Check(page.Model.Equipped(0,2)==-1,"Unequip failed");
            page.SelectSlot(3);await UniTask.Delay(350);equipment.OnEscape();await UniTask.Delay(350);Check(!equipment.IsOpen&&!page.pausePolicy.IsPaused,"Popup back left pause locked");
        ...
        {
            page.holdDig.HardCancel();if(equipment!=null&&equipment.IsOpen)equipment.OnClickClose();
